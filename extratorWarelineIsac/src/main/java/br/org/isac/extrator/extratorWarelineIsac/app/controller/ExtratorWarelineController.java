...
public class ExtratorWarelineController {
	...
	private PagtosWareline convertePagamentosPostPreToMySql(PagtosPostGre p) {
		...
		c.setNumatend(Integer.parseInt(p.getNumatend()));
		c.setNumbaiori(p.getNumbaiori());
		c.setNumconta(Integer.parseInt(p.getNumconta()));
		c.setNumcontbol(p.getNumcontbol());
		c.setNumdoc(p.getNumdoc());
		c.setNumentr(Integer.parseInt(p.getNumentr()));
		c.setNumnotas(p.getNumnotas());
		c.setNumpagto(Integer.parseInt(p.getNumpagto()));
		c.setNumparcori(p.getNumparcori());
		c.setNumpgtori(Integer.parseInt(p.getNumpgtori()));
		c.setNumprojeto(Integer.parseInt(p.getNumprojeto()));
