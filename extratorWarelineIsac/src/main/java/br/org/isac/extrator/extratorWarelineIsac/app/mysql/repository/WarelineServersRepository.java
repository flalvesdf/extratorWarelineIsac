...
//	public List<WarelineServers> getServers(@Param("ip") String iphost);
	...
	@Query("select u from WarelineServers u where u.unidade IN (:unidades) and u.ativo = 'S' order by u.unidade asc")
	public List<WarelineServers> getServersByUnidade(@Param("unidades") List<Integer> unidades);
...
//	public String getServerScheduled(@Param("ip") String ip);
	...
	@Query(value ="select pws.schedule from pt_wl_servidores pws where pws.unidade = :unidade and pws.ativo = 'S' LIMIT 1", nativeQuery = true)
	public String getServerScheduledByUnidade(@Param("unidade") Integer unidade);
